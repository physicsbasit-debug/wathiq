-- واثق 0.1.0 | المخطط الحالي الكامل
-- هذا الملف هو نقطة الإعداد الوحيدة لبيئة Supabase جديدة.
-- البيئة المنشورة التي سبق أن نفذت مخطط واثق لا تحتاج تشغيل هذا الملف من جديد.
...
create table if not exists public.source_registry (
  ...
  authority text not null check (authority in ('مصدر مرفوع', 'منهج عُماني', 'كامبريدج', 'مصدر عالمي')),
  title text not null,
  ...
  semester text not null default 'غير محدد'
  ...
  -- اسم العمود تاريخي للتوافق مع البيئة المنشورة؛ القيمة الحالية مسار منطقي داخل فهرس واثق وليست Google Drive.
  drive_path text not null,
  content_fingerprint text,
  file_size_bytes bigint,
  mime_type text,
  extraction_status text not null default 'لم يبدأ'
  ...
  detected_headings jsonb not null default '[]'::jsonb,
...
create policy "owner reads own source registry" on public.source_registry for select to authenticated using (owner_id = auth.uid());
drop policy if exists "owner inserts own source registry" on public.source_registry;
create policy "owner inserts own source registry" on public.source_registry for insert to authenticated with check (owner_id = auth.uid());
drop policy if exists "owner updates own source registry" on public.source_registry;
create policy "owner updates own source registry" on public.source_registry for update to authenticated using (owner_id = auth.uid()) with check (owner_id = auth.uid());
drop policy if exists "owner deletes own source registry" on public.source_registry;
create policy "owner deletes own source registry" on public.source_registry for delete to authenticated using (owner_id = auth.uid());
...
create index if not exists source_chunks_owner_source_idx on public.source_chunks(owner_id, source_id, chunk_index);
create index if not exists source_chunks_search_vector_idx on public.source_chunks using gin(search_vector);
...
create policy "owner reads own source chunks" on public.source_chunks for select to authenticated using (owner_id = auth.uid());
drop policy if exists "owner inserts own source chunks" on public.source_chunks;
create policy "owner inserts own source chunks" on public.source_chunks for insert to authenticated with check (owner_id = auth.uid());
drop policy if exists "owner updates own source chunks" on public.source_chunks;
create policy "owner updates own source chunks" on public.source_chunks for update to authenticated using (owner_id = auth.uid()) with check (owner_id = auth.uid());
drop policy if exists "owner deletes own source chunks" on public.source_chunks;
create policy "owner deletes own source chunks" on public.source_chunks for delete to authenticated using (owner_id = auth.uid());
...
create table if not exists public.source_ocr_pages (
  ...
  confidence real,
  provider text not null default 'gemini-ocr',
  processed_at timestamptz not null default now(),
  primary key (owner_id, source_id, page_number),
  foreign key (owner_id, source_id) references public.source_registry(owner_id, id) on delete cascade
);
create index if not exists source_ocr_pages_owner_source_idx on public.source_ocr_pages(owner_id, source_id, page_number);
...
create policy "owner reads own OCR pages" on public.source_ocr_pages for select to authenticated using (owner_id = auth.uid());
drop policy if exists "owner inserts own OCR pages" on public.source_ocr_pages;
create policy "owner inserts own OCR pages" on public.source_ocr_pages for insert to authenticated with check (owner_id = auth.uid());
drop policy if exists "owner updates own OCR pages" on public.source_ocr_pages;
create policy "owner updates own OCR pages" on public.source_ocr_pages for update to authenticated using (owner_id = auth.uid()) with check (owner_id = auth.uid());
drop policy if exists "owner deletes own OCR pages" on public.source_ocr_pages;
create policy "owner deletes own OCR pages" on public.source_ocr_pages for delete to authenticated using (owner_id = auth.uid());

...
export function reviewCompletedAssessment(
  ...
  const directRecall = results.filter((result) => /(ما المقصود|عرف|اكتب تعريف|اذكر وحده|حدد المصطلح)/u.test(
    normalizeArabic(`${result.content.stimulus} ${result.content.text}`),
  ));

import type { AssessmentEvidenceAnchor, AssessmentItemContract, AssessmentModelContent } from "./contracts.js";
import { sourceContentHash } from "./hashing.js";

export interface AssessmentEvidenceSegment {
...
export function normalizeArabicForGrounding(value: string): string {
  return value
    .normalize("NFKC")
    .replace(/[إأآٱ]/gu, "ا")
    .replace(/ى/gu, "ي")
...
export function groundingTokens(value: string): string[] {
...
export async function buildEvidenceSegments(content: string): Promise<AssessmentEvidenceSegment[]> {
  const sanitized = sanitizeSourceContent(content);
  if (!sanitized) throw new Error("مقطع المصدر فارغ بعد التنظيف.");
  ...
  for (const entry of rough) {
    if (entry.length <= 700) merged.push(entry);
    else {
      for (let start = 0; start < entry.length; start += 550) merged.push(entry.slice(start, start + 650).trim());
    }
  }
  const segments: AssessmentEvidenceSegment[] = [];
  ...
  if (!segments.length) throw new Error("لا يحتوي مقطع المصدر دليلًا نصيًا كافيًا لبناء السؤال.");
  return segments;
}

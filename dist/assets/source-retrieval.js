...
export function tokenizeArabicSearch(value) {
...
export function isLikelyNavigationOrMetadataChunk(content) {
    ...
    if (lessonCodes.length >= 4 && punctuation.length <= 5)
        return true;
    if (unitMentions.length >= 4 && punctuation.length <= 4)
        return true;
    return false;
}
export function referenceSupportsLesson(query, content) {
    if (isLikelyNavigationOrMetadataChunk(content))
        return false;
...
function scoreChunk(query, tokens, candidate) {
    const normalized = normalizeArabicSearchText(candidate.chunk.content);
    if (!normalized || !referenceSupportsLesson(query, candidate.chunk.content))
        return 0;
    let score = 0;
    const normalizedQuery = normalizeArabicSearchText(query);
    const corePhrase = tokens.join(" ");
    if (normalizedQuery.length >= 4 && normalized.includes(normalizedQuery))
        score += 60;
    else if (corePhrase.length >= 4 && normalized.includes(corePhrase))
        score += 42;
    for (const token of tokens) {
        const escaped = token.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
        const matches = normalized.match(new RegExp(`(?:^|\\s)${escaped}(?=\\s|$)`, "g"));
        if (matches?.length)
            score += 12 + Math.min(18, (matches.length - 1) * 4);
        else if (normalized.includes(token))
            score += 5;
    }
    if (tokens.length && tokens.every((token) => normalized.includes(token)))
        score += 20;
    if (candidate.chunk.content.length >= 500)
        score += 8;
    if ((candidate.chunk.content.match(/[.؟!؛]/g) ?? []).length >= 3)
        score += 5;
    return score + sourcePriority(candidate.source) / 10;
}
function excerptAroundMatch(content, tokens, maxLength = 300) {
...
export function rankSourceChunks(query, candidates, limit = 6) {
    const tokens = tokenizeArabicSearch(query);
    if (!tokens.length)
        return { references: [], matchedSourceCount: 0, searchedChunkCount: candidates.length };
...
//# sourceMappingURL=source-retrieval.js.map
export function interleaveAssessmentItems(items, isMultipleChoice) {
    const multipleChoice = items.filter(isMultipleChoice);
    const constructed = items.filter((item) => !isMultipleChoice(item));
    if (!multipleChoice.length || !constructed.length)
        return [...items];
    const result = [];
    let mcqIndex = 0;
...
export function hasAvoidableAdjacentMultipleChoice(items, isMultipleChoice) {
    ...
    if (multipleChoiceCount > constructedCount + 1)
        return false;
...
`;
export function buildStandaloneExamDocument(input) {
...
async function svgElementToPngDataUrl(svg) {
    ...
    try {
        ...
        if (!context)
            throw new Error("المتصفح لا يدعم تجهيز صور Word.");
...
async function imageUrlToDataUrl(url) {
    ...
    if (!response.ok)
        throw new Error(`تعذر تنزيل الصورة (${response.status}).`);
    const blob = await response.blob();
    if (!blob.type.startsWith("image/"))
        throw new Error("الملف المستلم ليس صورة صالحة.");
...
export function downloadBlob(fileName, blob) {
    const navigatorWithLegacySave = window.navigator;
...
export async function downloadWordHtml(fileName, html) {
...
//# sourceMappingURL=exam-export.js.map
import { ASSESSMENT_BLUEPRINT_VERSION, ASSESSMENT_CONTRACT_VERSION, ASSESSMENT_ENGINE_SCHEMA_VERSION, MODEL_ALLOWED_OUTPUT_FIELDS, MODEL_FORBIDDEN_OUTPUT_FIELDS, } from "./contracts.js";
...
export function canTransitionItemStatus(from, to) {
    return ITEM_TRANSITIONS[from].includes(to);
}
export function assertItemStatusTransition(from, to) {
    if (!canTransitionItemStatus(from, to)) {
        throw new AssessmentEngineError("INTERNAL_ERROR", `انتقال حالة مفردة غير مسموح: ${from} ← ${to}.`, { from, to });
    }
}
export function assertBlueprintIntegrity(blueprint) {
...
export function assertItemContractIntegrity(contract) {
...
export function assertModelOutputOwnership(value) {
    ...
    const allowed = new Set(MODEL_ALLOWED_OUTPUT_FIELDS);
    const unknown = Object.keys(record).filter((field) => !allowed.has(field));
    if (forbidden.length || unknown.length) {
        throw new AssessmentEngineError("MODEL_ASSESSMENT_MISMATCH", "أعاد النموذج حقولًا خارج عقد المحتوى المسموح.", { forbidden, unknown });
    }
}

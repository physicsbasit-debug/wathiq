function seededRange(seed, minimum, span, step) {
    return Number((minimum + (seed % Math.max(1, Math.floor(span / step) + 1)) * step).toFixed(3));
}
function quantity(kind, label, value, unit, direction) {
    return { kind, label, value, unit, direction };
}
function genericScientificItem(phenomenon, expectedResult) {
    return {
...
function visualSpec(contract, type, variant, extra = {}) {
    return {
        type,
...
function momentContract(contract) {
    ...
    return {
        ...
        scientificItem,
        visual: visualSpec(contract, contextual ? "context_scene" : "force_diagram", contextual ? contract.scenarioTarget : "moments", {
...
function forceContract(contract) {
...
function electrostaticContract(contract) {
...
function pressureContract(contract) {
...
function genericContract(contract) {
    let type = contract.visualTarget;
    let variant = contract.scenarioTarget === "scientific_abstract" ? "default" : contract.scenarioTarget;
    const extra = {};
    if (contract.scientificContractKey === "circuit") {
        type = "circuit_diagram";
        variant = "series_circuit";
        extra.components = ["battery", "switch_closed", "lamp", "ammeter"];
    }
    if (contract.scientificContractKey === "optics") {
        type = "ray_diagram";
        variant = "reflection";
        extra.values = [35, 35];
        extra.annotations = ["الشعاع الساقط", "العمود المقام", "الشعاع المنعكس"];
    }
    if (contract.scientificContractKey === "instrument") {
        type = "instrument_scale";
        variant = "measuring_cylinder";
        extra.values = [42];
        extra.annotations = ["قراءة التدريج = 42"];
    }
    if (contract.scientificContractKey === "graph") {
        type = "line_graph";
        variant = "trend";
        extra.xAxisLabel = "الزمن";
        extra.xAxisUnit = "s";
        extra.yAxisLabel = "الكمية المقاسة";
        extra.points = [{ x: 0, y: 1, label: "A" }, { x: 1, y: 3, label: "B" }, { x: 2, y: 5, label: "C" }];
    }
    if (contract.scientificContractKey === "table") {
        type = "data_table";
        variant = "table_comparison";
        extra.tableColumns = ["الحالة", "القيمة"];
        extra.tableRows = ["أ", "ب", "ج"];
        extra.tableCells = [["أ", "2"], ["ب", "4"], ["ج", "6"]];
    }
    if (contract.scientificContractKey === "process") {
        type = "flow_diagram";
        variant = "linear_flow";
        extra.labels = ["البداية", "التحول", "الناتج"];
    }
    return {
...
export function buildDeterministicScientificContract(contract) {
    if (contract.scientificContractKey === "moment")
        return momentContract(contract);
    if (contract.scientificContractKey === "force")
        return forceContract(contract);
    if (contract.scientificContractKey === "electrostatic")
        return electrostaticContract(contract);
    if (contract.scientificContractKey === "pressure")
        return pressureContract(contract);
    return genericContract(contract);
}
//# sourceMappingURL=scientific-contracts.js.map
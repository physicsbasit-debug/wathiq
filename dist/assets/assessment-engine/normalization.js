...
export function normalizeAssessmentModelContent(value, contract) {
    ...
    if (markScheme.length !== contract.marks) {
        throw new AssessmentEngineError("MODEL_ASSESSMENT_MISMATCH", "نموذج التصحيح لا يوزع نقطة مستقلة لكل درجة.", { expectedMarks: contract.marks, actualPoints: markScheme.length });
    }
    if (contract.questionType === "اختيار من متعدد") {
        if (options.length !== 4 || !options.includes(answer)) {
            throw new AssessmentEngineError("MODEL_ASSESSMENT_MISMATCH", "سؤال الاختيار من متعدد لا يحتوي أربعة بدائل مختلفة وإجابة مطابقة لها.");
        }
    }

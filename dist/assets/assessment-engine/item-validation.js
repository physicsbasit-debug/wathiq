...
export function validateAssessmentContentAgainstContract(content, contract, scientific) {
    ...
    if (content.markScheme.length !== contract.marks)
        throw new Error("عدد نقاط التصحيح لا يساوي درجة المفردة.");
    if (contract.questionType === "اختيار من متعدد") {
        if (content.options.length !== 4 || !content.options.includes(content.answer))
            throw new Error("بدائل سؤال الاختيار من متعدد أو إجابته غير صالحة.");
    }
    else if (content.options.length !== 0)
        throw new Error("السؤال الإنشائي لا يقبل بدائل اختيار.");
    if (scientific.expectedAnswerTokens.length) {
        ...
        if (missing.length)
            throw new Error(`الإجابة تخالف النتيجة الحتمية أو وحدتها: ${missing.join("، ")}.`);
    }
}

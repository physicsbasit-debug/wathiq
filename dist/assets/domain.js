...
export function suggestCountsForMarks(totalMarks, difficulty) {
...
function cognitiveCycle(difficulty) {
    if (difficulty === "سهل")
        return ["معرفة", "معرفة", "تطبيق"];
    if (difficulty === "متقدم")
        return ["تطبيق", "استدلال", "استدلال"];
    return ["معرفة", "تطبيق", "استدلال"];
}
function questionEntries(counts) {
    return [
        ...Array.from({ length: counts.mcq }, () => ({ type: "اختيار من متعدد", marks: 1 })),
        ...Array.from({ length: counts.short }, () => ({ type: "إجابة قصيرة", marks: 2 })),

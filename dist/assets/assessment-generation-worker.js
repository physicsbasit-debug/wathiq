export class AssessmentGenerationWorkerService {
    ...
    async processItemSynchronously(itemId) {
    ...
    async post(payload) {
        ...
        if (!response.ok)
            throw new Error(errorMessage(body, `تعذر الاتصال بعامل توليد المفردات (${response.status}).`));
        return body;
    }
}
function parseWorkerResponse(value, expectedItemId) {
...
function parseOutcome(record, expectedItemId) {
    const statuses = new Set(["ready", "retry_pending", "failed", "skipped", "stale"]);
    if (record.itemId !== expectedItemId || typeof record.status !== "string" || !statuses.has(record.status)) {
...
//# sourceMappingURL=assessment-generation-worker.js.map
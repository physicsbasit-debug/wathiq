...
function syncActiveView(view, replace = false) {
...
function renderSourceContextSummary() {
    const references = state.draft.sourceReferences;
    if (!references.length)
        return "";
    const primaryByLesson = new Map();
    for (const reference of references) {
        const lesson = reference.lessonTopic ?? "درس غير محدد";
        if (!primaryByLesson.has(lesson))
            primaryByLesson.set(lesson, reference);
    }
...
function visualJobMessage(job) {
    if (!job)
        return "لم تُنشأ مهمة الصورة بعد.";
    if (job.status === "queued")
        return "الصورة في طابور التنفيذ الدائم.";
    if (job.status === "generating")
        return `يجري توليد الأصل البصري 2D، المحاولة ${job.attemptCount} من ${job.maxAttempts}.`;
    if (job.status === "validating")
        return "تم توليد الصورة ويجري الآن فحصها علميًا وبصريًا.";
    if (job.status === "retry_pending")
        return job.errorMessage || "سيعيد واثق محاولة الصورة تلقائيًا.";
    if (job.status === "ready")
        return "اكتمل الأصل البصري واعتمد علميًا، وهو المستخدم في المعاينة وWord وPDF.";
    if (job.status === "failed")
        return job.errorMessage || "فشلت مهمة الصورة بعد استنفاد المحاولات.";
...
function renderPaperPrompt(item, proposal, label, subpart) {
...
function buildPaperLayout(selected) {
    const labels = new Map();
    const ordered = interleaveAssessmentItems(selected, ({ item }) => item.questionType === "اختيار من متعدد");
    let mainNumber = 1;
    const parts = [];
    for (let index = 0; index < ordered.length;) {
        ...
        while (cursor < ordered.length) {
            ...
            if (!next || next.item.questionType === "اختيار من متعدد" || (next.item.lessonLabel || "مفردات مترابطة") !== lessonLabel)
                break;
            group.push(next);
            cursor += 1;
        }
...
function renderAnswerKeyArticles(selected, labels, exportMode = false) {
...
function renderAnswerKey(selected, labels) {
...
function renderStudentPaper(subject, paperLayout) {
...
async function verifyRequiredVisualAssetsForExport() {
    ...
    if (urls.some((url) => !url))
        throw new Error("تعذر التصدير لأن أحد الأصول البصرية المطلوبة غير مرتبط بالمفردة.");
    await Promise.all([...new Set(urls)].map(async (url) => {
        const response = await fetch(url, { mode: "cors", credentials: "omit" });
        if (!response.ok)
            throw new Error(`تعذر الوصول إلى أحد الأصول البصرية (${response.status}).`);
        const blob = await response.blob();
        if (!blob.type.startsWith("image/") || blob.size === 0)
            throw new Error("أحد الأصول البصرية المحفوظة ليس صورة صالحة للتصدير.");
    }));
}
async function executeExamExport(action, approved) {
    try {
        await verifyRequiredVisualAssetsForExport();
        const kind = action.includes("answer") ? "answer" : "student";
...
function renderPolicyReference() {
    ...
    if (!grades58 || !grade9 || !grade10 || !final58 || !final9 || !final10)
        throw new Error("تعذر تحميل مرجع تقويم العلوم.");
    return `
...
function renderPolicySpecCard(label, spec) {
...
function renderPolicyRuleCard(title, rules) {
...
function renderSourceStoragePanel() {
...
function renderPendingSourceUpload() {
...
function renderSourceIndexingProgress() {
...
function renderAdmin() {
    ...
    <section class="source-table-wrap">
      ...
      <div class="source-filter-row">${["الكل", "جاهز للفهرسة", "مفهرس", "يحتاج مراجعة", "مؤرشف"].map((filter) => `<button class="filter-chip ${state.sourceFilter === filter ? "active" : ""}" data-source-filter="${filter}">${filter}</button>`).join("")}</div>
...
function renderSourceForm() {
    ...
    const availableSourceSubjects = draft.grade
        ? SUBJECTS.filter((subject) => subject.grades.includes(draft.grade))
        : SUBJECTS;
    const issueFor = (field) => validation.issues.find((issue) => issue.field === field)?.message ?? "";
    return `
...
function renderSourceReadinessPanel(source) {
...
function sourceExtractionActionLabel(source, busy) {
    if (busy)
        return source.extractionVersion?.startsWith("google-cloud-vision") ? "جارٍ OCR…" : "جارٍ الاستخراج…";
    if (source.extractionStatus === "يحتاج OCR")
        return "تشغيل OCR العربي";
    if (source.extractionVersion?.startsWith("google-cloud-vision-ocr-pending"))
        return "استكمال OCR";
    if (source.extractionVersion?.startsWith("google-cloud-vision"))
        return "إعادة OCR";
    if (source.extractionStatus === "فشل")
        return "إعادة المحاولة";
    if (source.extractionStatus === "مكتمل")
        return "إعادة الفهرسة";
    return "استخراج وفهرسة";
}
function extractionStatusSlug(status) {
    if (status === "مكتمل")
        return "done";
    if (status === "جارٍ الاستخراج")
        return "busy";
    if (status === "يحتاج OCR" || status === "فشل")
        return "review";
    return "idle";
}
function sourceStatusSlug(status) {
    if (status === "مفهرس")
        return "indexed";
...
function handleAction(action, element) {
    ...
    if (["library-export-student-word", "library-export-student-pdf", "library-export-answer-word", "library-export-answer-pdf"].includes(action)) {
        ...
        if (!reviewReadiness(selectedPaperItems()).ready)
            return showToast("لا يمكن التصدير قبل اكتمال الأصول البصرية وجميع فحوص المراجعة.");
    ...
    if (["sync-visual-job", "retry-visual-job", "regenerate-visual-job"].includes(action)) {
        ...
        if (planItemId)
            void retryVisualJob(planItemId);
        return;
    }
    if (action === "save-now")
        return saveNow();
    ...
    if (action === "export-source-registry")
    ...
    if (action === "archive-source" && sourceId) {
        void archiveSource(sourceId);
        return;
    }
    if (action === "restore-source" && sourceId) {
        void restoreSource(sourceId);
        return;
    }
    if (action === "index-source" && sourceId) {
        void extractAndIndexSource(sourceId);
        return;
    }
}
function visualJobSubject() {
    return SUBJECTS.find((entry) => entry.id === state.draft.subjectId)?.label ?? state.draft.subjectId;
}
function invalidateVisualJobForPlanItem(planItemId) {
    delete state.draft.visualJobs[planItemId];
    const item = state.draft.plan.find((entry) => entry.id === planItemId);
    if (item?.visual?.illustration)
        item.visual = stripQuestionVisualIllustration(item.visual);
}
function applyVisualJobSnapshots(jobs) {
    let changed = false;
...
function stopVisualJobPolling() {
    if (visualJobPollTimer)
        window.clearTimeout(visualJobPollTimer);
    visualJobPollTimer = undefined;
}
function scheduleVisualJobPolling() {
    stopVisualJobPolling();
    if (!hasPendingVisualJobs() || !visualJobService || state.sourceStorageStatus !== "متصل")
        return;
...
async function syncVisualJobs(enqueueRequired) {
    if (!visualJobService || !centralSourceStore?.currentSession || state.sourceStorageStatus !== "متصل" || state.draft.grade === null)
        return;
...
async function retryVisualJob(planItemId) {
    if (!visualJobService || state.draft.status === "معتمد")
        return;
...
async function nextStep() {
    ...
    if (step === 3) {
        ...
        if (state.draft.status !== "معتمد")
            state.draft.status = "جاهز للمراجعة";
        return setStep(4);
    }
}
...
async function prepareSourceContext() {
    ...
    syncDraftTopicFromLessons(state.draft);
    const eligible = state.sources.filter((source) => source.grade === state.draft.grade &&
        source.subjectId === state.draft.subjectId &&
        source.status === "مفهرس" &&
        source.extractionStatus === "مكتمل");
    if (!eligible.length) {
    ...
    try {
        ...
        const candidates = chunkGroups.flatMap(({ source, chunks }) => chunks.map((chunk) => ({ source, chunk })));
...
function syncSetupFieldsFromDom() {
    ...
    if (dateInput?.value)
        state.draft.examDate = dateInput.value;
    const schoolInput = document.querySelector("#school-input");
    if (schoolInput)
        state.draft.school = schoolInput.value;
    const directorateInput = document.querySelector("#directorate-input");
    if (directorateInput)
        state.draft.directorate = directorateInput.value;
    const academicYearInput = document.querySelector("#academic-year-input");
    if (academicYearInput)
        state.draft.academicYear = academicYearInput.value;
    const semesterSelect = document.querySelector("#semester-select");
    if (semesterSelect)
        state.draft.semester = semesterSelect.value;
}
function bindSetupStep() {
    document.querySelector("#exam-title-select")?.addEventListener("change", (event) => {
...
function bindAdmin() {
    ...
    document.querySelector("#source-kind")?.addEventListener("change", (event) => {
        state.sourceDraft.kind = event.target.value;
        render();
    });
    document.querySelector("#source-semester")?.addEventListener("change", (event) => {
        state.sourceDraft.semester = event.target.value;
        render();
    });
    document.querySelector("#source-grade")?.addEventListener("change", (event) => {
        const value = event.target.value;
        state.sourceDraft.grade = value ? Number(value) : null;
        const subjectStillValid = SUBJECTS.some((subject) => subject.id === state.sourceDraft.subjectId && state.sourceDraft.grade !== null && subject.grades.includes(state.sourceDraft.grade));
        if (!subjectStillValid)
            state.sourceDraft.subjectId = "";
        render();
    });
    document.querySelector("#source-subject")?.addEventListener("change", (event) => {
        state.sourceDraft.subjectId = event.target.value;
        render();
    });
    ...
    document.querySelector("#source-rights")?.addEventListener("change", (event) => {
        state.sourceDraft.rightsConfirmed = event.target.checked;
    });
    document.querySelector("#source-registry-file")?.addEventListener("change", async (event) => {
...
function bindSourceTextInput(id, key) {
    document.querySelector(`#${id}`)?.addEventListener("input", (event) => {
        state.sourceDraft[key] = event.target.value;
    });
}
async function saveSourceFromForm() {
    ...
    if (state.sourceDraft.mode === "url") {
        ...
        if (state.sourceStorageStatus === "متصل")
            await persistSourcesCentrally([source], "تم حفظ الرابط في السجل المركزي.");
        else
            showToast("تم حفظ الرابط محليًا، وسيُنقل إلى السجل المركزي بعد الاتصال.");
        return;
    }
    ...
    render();
    try {
        const uploaded = await googleDriveService.uploadPdfSource({ ...source, subjectLabel: sourceSubjectLabel(source.subjectId) }, state.sourceFile, updateSourceUploadProgress);
...
function updateSourceUploadProgress(progress) {
    ...
    const label = document.querySelector(".source-upload-progress strong");
    const percent = document.querySelector(".source-upload-progress > div > span");
    const bar = document.querySelector(".source-upload-progress .upload-progress-track span");
    if (label)
...
async function cancelPendingSourceUpload() {
    if (!googleDriveService)
        return;
    if (!window.confirm("سيُلغى الرفع غير المكتمل فقط، ولن يُحذف أي مصدر مكتمل. هل تريد المتابعة؟"))
        return;
    try {
...
async function extractAndIndexSource(sourceId) {
    ...
    try {
        ...
        if (useOcr) {
            ...
            await centralSourceStore.updateExtractionState(sourceId, "جارٍ الاستخراج", "جارٍ تشغيل OCR العربي عبر Google Cloud Vision مع حفظ كل صفحة للاستكمال بعد الانقطاع.", "google-cloud-vision-ocr-pending-1");
            const existingPages = await centralSourceStore.listOcrPages(sourceId);
            result = await extractPdfWithArabicOcr(sourceId, access, existingPages, ({ sourceId: requestSourceId, pageNumber, totalPages, image }) => googleDriveService.ocrSourcePage(requestSourceId, pageNumber, totalPages, image), (progress) => updateSourceIndexingProgress(progress));
        }
        else {
...
function updateSourceIndexingProgress(progress) {
    state.sourceIndexingProgress = progress.percent;
    state.sourceIndexingMessage = progress.message;
    const message = document.querySelector(".source-indexing-card p");
    const value = document.querySelector(".source-indexing-card > strong");
    const bar = document.querySelector(".source-indexing-card .upload-progress-track span");
    if (message)
...
async function importSourceRegistry(file) {
...
function updateSourceStatus(sourceId, status, message) {
    ...
    if (state.sourceStorageStatus === "متصل" && centralSourceStore && updated) {
        void centralSourceStore.updateStatus(sourceId, status, updated.updatedAt)
            ...
            .catch((error) => markCentralStorageError(error));
    }
    else {
        showToast(message);
    }
...
async function loadAndSyncCentralSources() {
    ...
    try {
        ...
        if (localSources.length)
            await centralSourceStore.upsertSources(localSources);
...
async function repairLegacyLowQualityExtractions(sources) {
    if (!centralSourceStore)
        return sources;
    const candidates = sources.filter((source) => (source.mode === "file"
        && source.extractionStatus === "مكتمل"
...
async function persistSourcesCentrally(sources, successMessage) {
    if (!centralSourceStore?.currentSession)
        return;
    state.sourceStorageBusy = true;
    render();
...
function resetGoogleDriveState(status, message) {
...
function applyGoogleDriveStatus(status) {
    if (!status.connected) {
...
async function connectGoogleDrive() {
    if (!googleDriveService || state.sourceStorageStatus !== "متصل")
        return;
...
async function verifyGoogleDriveFolders() {
    if (!googleDriveService || state.driveStatus !== "متصل")
        return;
    state.driveBusy = true;
    render();
    try {
...
async function disconnectGoogleDrive() {
    if (!googleDriveService)
        return;
    if (!window.confirm("سيُفصل اتصال Google Drive فقط، ولن تُحذف المجلدات أو الملفات. هل تريد المتابعة؟"))
        return;
    state.driveBusy = true;
    render();
    try {
...
function consumeGoogleDriveCallback() {
...
function markCentralStorageError(error) {
...
async function bootstrapCentralStorage() {

export const QUESTION_VISUAL_TYPES = [
    "none",
    "context_scene",
    "line_graph",
    "bar_chart",
...
export const QUESTION_VISUAL_VARIANTS = [
    "default",
    "door_handle",
    "playground_seesaw",
    "wrench_tool",
...
export const QUESTION_VISUAL_ROLES = ["read", "calculate", "interpret", "compare", "complete", "draw", "evaluate"];
const VISUAL_LABELS = {
    none: "دون رسم",
...
export function questionVisualTypeLabel(type) {
    return VISUAL_LABELS[type];
}
export function isQuestionVisualType(value) {
    return typeof value === "string" && QUESTION_VISUAL_TYPES.includes(value);
}
export function isQuestionVisualVariant(value) {
    return typeof value === "string" && QUESTION_VISUAL_VARIANTS.includes(value);
}
export function isQuestionVisualRole(value) {
    return typeof value === "string" && QUESTION_VISUAL_ROLES.includes(value);
}
export function isCircuitComponent(value) {
    return typeof value === "string" && CIRCUIT_COMPONENTS.includes(value);
}
...
export function parseQuestionVisualIllustration(value) {
...
function cleanTextArray(value, maxItems, maxLength = 80) {
...
function cleanNumberArray(value, maxItems) {
    if (!Array.isArray(value))
        return [];
    return value
...
function cleanStringMatrix(value, maxRows = 8, maxColumns = 6) {
    if (!Array.isArray(value))
        return [];
...
export function emptyQuestionVisualSpec() {
    return {
        type: "none",
        visualId: "",
        variant: "default",
...
export function parseQuestionVisualSpec(value, expectedType) {
    const record = asRecord(value);
...
export function diversifyQuestionVisualSpec(spec, index, planItemId = "") {
    if (spec.type === "none")
        return { ...spec, visualId: spec.visualId || "", variant: spec.variant ?? "default" };
    const contextVariants = ["door_handle", "playground_seesaw", "wrench_tool", "bicycle_brake", "shopping_trolley", "school_bag", "water_tank", "solar_panel", "laboratory_setup", "road_safety"];
    const pressureVariants = ["submerged_object", "depth_comparison", "force_area", "liquid_column"];
    const circuitVariants = ["series_circuit", "measurement_circuit"];
    const electrostaticVariants = ["charge_transfer", "attraction_repulsion", "electric_field"];
    const graphVariants = ["trend", "comparison", "multi_series"];
    const tableVariants = ["table_completion", "table_comparison"];
    const scaleVariants = ["thermometer", "burette", "measuring_cylinder", "meter_scale"];
    const rayVariants = ["reflection", "refraction", "converging_lens", "prism"];
    const forceVariants = ["free_body", "balanced_forces", "moments"];
    const flowVariants = ["linear_flow", "cycle_flow", "state_change"];
...
export function validateQuestionVisualSpec(spec) {
    if (spec.type === "none")
        return;
    if (!spec.title || !spec.altText)
        throw new Error("الرسم التعليمي يحتاج عنوانًا ووصفًا بديلًا.");
    ...
    if (spec.type === "bar_chart") {
        if (!spec.yAxisLabel || spec.yMax <= spec.yMin)
            throw new Error("رسم الأعمدة يحتاج محورًا رأسيًا صالحًا.");
    ...
    if (spec.type === "circuit_diagram") {
        ...
        if (!spec.components.includes("battery"))
            throw new Error("الدائرة الكهربائية تحتاج بطارية.");
    ...
    if (spec.type === "instrument_scale") {
        ...
        if (spec.values.length < 4)
            throw new Error("تدريج الجهاز يحتاج الحد الأدنى والأعلى والخطوة والقراءة.");
    ...
    if (spec.type === "ray_diagram") {
        ...
        if (spec.values.length < 2)
            throw new Error("مخطط الأشعة يحتاج قيمًا هندسية أساسية.");
        return;
    }
...
function axisCaption(label, unit) {
    return unit ? `${label} (${unit})` : label;
}
function renderLineGraph(spec) {
    const width = 640;
    const height = 380;
    const left = 78;
    const right = 34;
    ...
    const x = (value) => left + ((value - spec.xMin) / (spec.xMax - spec.xMin)) * plotWidth;
    const y = (value) => top + plotHeight - ((value - spec.yMin) / (spec.yMax - spec.yMin)) * plotHeight;
...
function renderPressureDiagram(spec) {
    const width = 640;
    const height = 360;
...
function renderInstrumentScale(spec) {
    const width = 640;
    const height = 380;
...
function renderRayDiagram(spec) {
    const width = 640;
    const height = 360;
...
export function isAiIllustrationEligible(spec) {
    if (spec.type === "context_scene") {
...
function renderContextScene(spec) {
    const width = 640;
    const height = 360;
    const title = `<text x="${width / 2}" y="28" class="qv-title" text-anchor="middle">${escapeXml(spec.title)}</text>`;
    const label = (x, y, value) => `<text x="${x}" y="${y}" class="qv-annotation" text-anchor="middle">${escapeXml(value)}</text>`;
...
function forceDiagramObjectKind(spec) {
    const material = `${spec.title} ${spec.altText} ${spec.labels.join(" ")}`.toLowerCase();
    if (/(عربه|عربة|تسوق|trolley|cart)/u.test(material))
        return "trolley";
    if (/(حقيبه|حقيبة|مدرسيه|مدرسية|bag)/u.test(material))
        return "school_bag";
    return "crate";
}
function renderForceDiagramObject(spec) {
...
function renderForceDiagramOverlaySvg(spec) {
    const width = 640;
    const height = 360;
...
function renderForceDiagram(spec) {
    const width = 640;
    const height = 360;
...
function renderFlowDiagram(spec) {
    ...
    const node = (x, y, label, index) => `<rect x="${x - 62}" y="${y - 25}" width="124" height="50" rx="12" class="qv-flow-node"/><text x="${x}" y="${y + 5}" class="qv-flow-text" text-anchor="middle">${escapeXml(label || `المرحلة ${index + 1}`)}</text>`;
...
function circuitSymbol(component, x, y) {
    switch (component) {
        case "battery":
...
export function renderQuestionVisualSvg(spec) {
    validateQuestionVisualSpec(spec);
    if (spec.type === "none")
        return "";
...
//# sourceMappingURL=question-visual.js.map